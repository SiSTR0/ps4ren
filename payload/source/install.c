...
int install_payload(struct thread *td, uint64_t kernbase, void *payload, size_t psize) {
	...
	if (elf_mapped_size(payload, &msize)) {
		printf("[ps4ren] install_payload: elf_mapped_size failed!\n");
		return 1;
	}
	...
	*(uint8_t *)(kernbase + 0xFCD56) = 7; // VM_PROT_ALL;
	...
	if (!payloadbase) {
		printf("[ps4ren] install_payload: kmem_alloc failed!\n");
	...
	*(uint8_t *)(kernbase + 0xFCD56) = 3; // VM_PROT_DEFAULT;
	...
	if ((r = load_elf(payload, psize, payloadbase, msize, (void **)&payload_entry))) {
		printf("[ps4ren] install_payload: load_elf failed (r: %i)!\n", r);
		return 1;
	}
	...
	printf("[ps4ren] payload loaded - 0x%llX\n", payloadbase);
